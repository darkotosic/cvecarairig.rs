...
export default function Home() {
  ...
  return <main>
    ...
    <section className="bg-primary text-white"><div className="mx-auto grid max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-20"><div><p className="text-sm font-semibold uppercase tracking-wide text-gold">{store.brand}</p><h1 className="mt-4 text-4xl font-bold sm:text-6xl">Cveće za svaki važan trenutak u Irigu i okolini.</h1><p className="mt-5 max-w-xl text-lg leading-8 text-surface-soft">Sveži buketi i cvetni aranžmani sa brzom potvrdom porudžbine i lokalnom dostavom.</p><div className="mt-8 flex flex-wrap gap-3"><Link href="/products" className="bg-paper px-5 py-3 font-semibold text-primary">Pogledaj aranžmane</Link><Link href="/shipping" className="border border-white/40 px-5 py-3 font-semibold">Dostava</Link></div></div>
      <div className="grid grid-cols-2 gap-3" aria-label="Izdvojeni cvetni aranžmani">{heroProducts.map((product, index) => <article key={product.id} className="overflow-hidden rounded-2xl bg-paper text-primary shadow-lg"><Link href={`/products/${product.slug}`} className="group block"><img src={product.image} width="1254" height="1254" alt={`${product.name} — cvetni aranžman sa dostavom u Irigu`} loading="eager" fetchPriority={index === 0 ? 'high' : 'auto'} decoding={index === 0 ? 'sync' : 'async'} className="aspect-square w-full object-cover transition group-hover:scale-[1.02]" /><h2 className="px-3 py-3 text-sm font-semibold sm:text-base">{product.name}</h2></Link></article>)}</div>
    </div></section>
    <section className="border-b bg-surface-soft"><div className="mx-auto grid max-w-7xl gap-3 px-4 py-8 sm:grid-cols-2 sm:px-6 lg:grid-cols-4 lg:px-8">{trust.map((item) => <div key={item} className="rounded-2xl bg-paper p-4 font-semibold text-primary shadow-sm ring-1 ring-line">{item}</div>)}</div></section>
    ...
    <section className="bg-surface-soft"><div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8"><h2 className="text-3xl font-bold text-primary">Kategorije</h2><div className="mt-6 grid gap-4 sm:grid-cols-2">{categories.map((category) => <Link key={category.id} href={`/products?category=${category.slug}`} className="rounded-3xl bg-paper p-6 font-semibold text-primary shadow-sm ring-1 ring-line">{category.name}</Link>)}</div></div></section>
  </main>;
}
