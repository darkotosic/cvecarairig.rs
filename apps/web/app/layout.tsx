  ...
  return (
    <html lang="sr">
      <body className="min-h-screen font-sans antialiased">
        ...
        <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
          <nav className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-slate-700">
              ...
              <Link href="/flower-care" className="hover:text-primary">
                Nega cveća
              </Link>
              <a
                href={`tel:${normalizeTelHref(GLOBAL_CTA_PHONE)}`}
        ...
        {children}
        <StickyCallButton phone={GLOBAL_CTA_PHONE} />
          ...
          <div className="mx-auto grid max-w-7xl gap-6 px-4 py-10 text-sm text-slate-600 sm:px-6 md:grid-cols-4 lg:px-8">
            ...
            <div className="flex flex-col gap-2">
              {settings.instagram_url && (
                <a href={settings.instagram_url} rel="noreferrer" target="_blank">
                  Instagram
                </a>
              )}
              ...
              {!contactEmail && !settings.store_phone && (
                <span>Kontakt podaci još nisu podešeni.</span>
              )}
            </div>
          </div>
        </footer>
